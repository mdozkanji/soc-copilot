...
- **LLM**: Claude via the Anthropic API, using native tool use (function calling). This is also a chance to get hands-on with the same agent-architecture skills (tool use, structured output, agent loops) that the AI-SOC vendors above are built on. *(Week 4 follow-up: switched to Groq's free tier -- Llama 3.3 70B, OpenAI-compatible tool calling -- specifically so the project runs as a portfolio demo without requiring anyone, including a reviewer cloning the repo, to pay for API access. The agent loop itself was refactored to be provider-agnostic, `src/soc_copilot/agent/llm_types.py`, so this kind of swap is a one-file change going forward, not a rewrite. See `devlog/0005-*.md`.)*
