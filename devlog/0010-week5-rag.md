...
## A harmless warning I didn't chase further
`chromadb` logs a `DeprecationWarning` about an unregistered custom embedding function during tests (`tests/test_chroma_retriever.py`'s fake). Tried the documented fixes (`get_config`, `build_from_config`); one internal registry warning remains. Confirmed it doesn't affect correctness (121/121 still pass) and stopped there — chasing every warning from a fake test double built to sidestep a real network constraint has diminishing returns.
